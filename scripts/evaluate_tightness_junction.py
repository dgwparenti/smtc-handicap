#!/usr/bin/env python3
"""Fit the Bayesian handicap model for JUNCTION and evaluate net-time tightness vs committee.

This script:
1. Fits the JUNCTION model with min_runs=50
...
5. Prints PASS if avg model range < 1.0s AND model tighter in >=70% of races, else FAIL
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Quantile shift multiplier (default: -1.30)",
    )
    parser.add_argument(
        "--max-shift",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Scale factor for handicap differences (default: 1.0)",
    )
    parser.add_argument(
        "--handicap-power",
        type=float,
        default=None,
        help="Power-law exponent for concave compression (default: 1.0, <1 compresses large handicaps)",
    )
    args = parser.parse_args()

    print("=" * 70)
    print(f"TIGHTNESS EVALUATION — JUNCTION Handicap Model (min_runs={MIN_RUNS})")
    print("=" * 70)
    ...
    print(
        ...
        f"handicap_scale={args.handicap_scale or 1.0}, "
        f"handicap_power={args.handicap_power or 1.0}"
    )
    ...
    for race_id, group in race_groups:
        ...
        hcap_df = calculate_handicaps(
            ...
            handicap_scale=args.handicap_scale if args.handicap_scale is not None else 1.0,
            handicap_power=args.handicap_power if args.handicap_power is not None else 1.0,
        )
